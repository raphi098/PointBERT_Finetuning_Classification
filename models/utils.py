...
def adjust_bn_momentum(module, momentum=0.01):
    if isinstance(module, (nn.BatchNorm1d, nn.BatchNorm2d)):
        module.momentum = momentum

def inplace_relu(m):
    """Set ReLU layers to operate in-place to save memory."""
...
def to_categorical(y: torch.Tensor, num_classes: int) -> torch.Tensor:
    """
    One-hot encodes a tensor on CPU or GPU.

    Args:
        y (torch.Tensor): Tensor of integer class labels, shape (N,).
